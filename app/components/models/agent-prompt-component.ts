  ...
  private loadEnvironmentConfig(): Record<string, any> {
    const defaultConfig: any = {
      ...
      langchainjs_compatibility: {
        export_format: this.options.langchainjsExport,
        flatten_metadata: this.options.flattenMetadata,
        runnable_interface: this.options.enableRunnableInterface,
      },
    };
    return loadEnvConfig(this.options.vercelMode, "LANGFLOW_AGENT_PROMPT_CONFIG", defaultConfig);
  }
  ...
  private buildPrompt(): Message {
    ...
    return {
      ...
      metadata: {
        ...
        langchainjs_ready: this.options.langchainjsExport,
        runnable_interface: this.options.enableRunnableInterface,
        vercel_mode: this.options.vercelMode,
        flatten_metadata: this.options.flattenMetadata,

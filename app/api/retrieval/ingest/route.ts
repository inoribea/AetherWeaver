...
export async function POST(req: NextRequest) {
  try {
    ...
    if (availableProviders.length === 0) {
      return NextResponse.json(
        { 
          error: "No embedding providers available. Please configure CLOUDFLARE_API_TOKEN + CLOUDFLARE_ACCOUNT_ID or OPENAI_API_KEY.",
          availableProviders: []
        },
        { status: 400 }
      );
    ...
    if (globalVectorStore) {
      ...
      provider = getBestEmbeddingProvider();
      ...
      console.log(`Created new vector store with ${provider.name} embeddings`);
    }
    ...
    return NextResponse.json({
      ...
      details: {
        ...
        embeddingModel: provider?.name === "Cloudflare" 
